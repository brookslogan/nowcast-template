...
A simple wrapper for the `datasetname_nowcasts` table in the Delphi database.
...
Nowcasts (value and standard deviation) are stored in the `datasetname_nowcasts` table.
+----------+-------------+------+-----+---------+----------------+
...
class NowcastsTable(DelphiDatabase.Table):
  """A database wrapper for the `datasetname_nowcasts` table."""

  SQL_INSERT = '''
    INSERT INTO `datasetname_nowcasts`
