...
A simple wrapper for the `datasetname_sensors` table in the Delphi database.
...
`datasetname_sensors` is the table where the data is stored.
...
# first party
from delphi.nowcast_datasetname.util.delphi_database import DelphiDatabase
...
class SensorsTable(DelphiDatabase.Table):
  ...
  SQL_INSERT = '''
    INSERT INTO
      `datasetname_sensors` (`target`, `name`, `location`, `epiweek`, `value`)
    VALUES
      (%s, %s, %s, %s, %s)
