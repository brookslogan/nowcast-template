...
class NowcastExperiment:
  ...
  class Provider:
    ...
    def get_data_source(self, epidata, sensors, locations, target):
      """Return a DatasetnameDataSource instance."""
      return DatasetnameDataSource(epidata, sensors, locations, target)
  ...
  @staticmethod
  def new_instance(target="ov_datasetname_broad"):
    """Return a production-ready instance."""
    return NowcastExperiment(
        NowcastExperiment.Provider(), Epidata, DatasetnameDataSource.new_instance(target))
  ...
  def get_locations_at_resolution(resolution):
    ...
    if resolution == 'state':
      locations = DatasetnameDataSource.ALL_LOCATIONS
    else:
      raise Exception("Only state resolution is currently supported for datasetname nowcasts.")
  ...
  def get_weeks_in_nowcast(self, sensor, min_observations):
    ...
    response = self.epidata.datasetname_sensors(
        secrets.api.datasetname_sensors, sensor, 'ca', week_range)
  ...
  def get_ablation_parameters(self, ablate):
    ...
    # method
    if ablate not in DatasetnameDataSource.SENSORS:
      raise UnknownSensorException('unknown sensor: %s' % ablate)
    sensors = [s for s in DatasetnameDataSource.SENSORS if s != ablate]
    locations = DatasetnameDataSource.ALL_LOCATIONS
  ...
  #   sensors = DatasetnameDataSource.SENSORS
...
def get_argument_parser():
  ...
  parser.add_argument(
      '--ablate',
      choices=DatasetnameDataSource.SENSORS,
