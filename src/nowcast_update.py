...
from delphi.nowcast_datasetname.fusion.nowcast import Nowcast
...
from delphi.nowcast_datasetname.util.nowcasts_table import NowcastsTable
...
class NowcastUpdate:
  ...
  @staticmethod
  def new_instance(target, test_mode):
    """
    ...
    data_source = DatasetnameDataSource.new_instance(target)
  ...
  def update(self, first_week, last_week):
    ...
    nowcasts = Nowcast(self.data_source, DatasetnameLocationMapper).batch_nowcast(weeks)
...
def get_argument_parser():
  ...
  parser.add_argument(
      '--target',
      type=str,
      default='datasetname_rate',
      help='The name of the datasetname sensor&nowcasting target (e.g., datasetname_rate)')
  parser.add_argument(
      '--test',
      action='store_true',
...
def validate_args(args):
  ...
  if (args.first is None) or (args.last is None):
    raise Exception('`first` and `last` must be specified in the current version of this script')
  if args.first and args.first > args.last:
    raise Exception('`first` must be less than or equal to `last`')
  return args.first, args.last, args.target, args.test


def main(first, last, target, test):
  """Run this script from the command line."""
  NowcastUpdate.new_instance(target, test).update(first, last)
