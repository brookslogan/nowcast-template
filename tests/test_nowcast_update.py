...
__test_target__ = 'delphi.nowcast_datasetname.nowcast_update'
...
class UnitTests(unittest.TestCase):
  ...
  def test_validate_args(self):
    ...
    with self.subTest(name='first < last'):
      ...
      self.assertEqual(validate_args(args), (1, 2, 'tar', False))
  ...
  def test_update(self):
    ...
    self.assertIn(('datasetname_rate', 201512, 'pa'), target_epiweek_location_triplets)
    self.assertIn(('datasetname_rate', 201513, 'pa'), target_epiweek_location_triplets)
    self.assertIn(('datasetname_rate', 201512, 'va'), target_epiweek_location_triplets)
    self.assertIn(('datasetname_rate', 201513, 'va'), target_epiweek_location_triplets)
