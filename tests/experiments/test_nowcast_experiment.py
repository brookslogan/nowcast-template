...
__test_target__ = 'delphi.nowcast_datasetname.experiments.nowcast_experiment'
...
class UnitTests(unittest.TestCase):
  ...
  def test_default_provider(self):
    ...
    self.assertIsInstance(data_source, DatasetnameDataSource)
  ...
  def test_get_locations_at_resolution(self):
    ...
    with self.subTest(name='state'):
      ...
      self.assertEqual(locations, DatasetnameDataSource.ALL_LOCATIONS)
  ...
  def test_get_weeks_in_nowcast(self):
    ...
    with self.subTest(name='enough historical data'):
      ...
      result = self.experiment.get_weeks_in_nowcast('isch', 2)
    ...
    with self.subTest(name='not enough historical data'):
      ...
      with self.assertRaises(InsufficientDataException):
        self.experiment.get_weeks_in_nowcast('isch', 2)
  ...
  def test_get_ablation_parameters(self):
    ...
    params = self.experiment.get_ablation_parameters('isch')
    sensors, locations, weeks, cov_impl = params

    actual_sensors = set(sensors)
    expected_sensors = set(DatasetnameDataSource.SENSORS) - {'isch'}
  ...
  #   self.assertEqual(sensors, DatasetnameDataSource.SENSORS)
  #   self.assertEqual(locations, Locations.nat_list)
  ...
  #   self.assertEqual(set(sensors), set(['cdc', 'isch', 'twtr']))
  #   self.assertEqual(locations, Locations.nat_list)
  ...
  def test_get_values_for_experiment(self):
    ...
    with self.subTest(name='vanilla'):
      ...
      self.assertEqual(len(params), 4)
      self.assertEqual(params[0], DatasetnameDataSource.SENSORS)
...
# todo review, fix, and restore commented out tests
