...
__test_target__ = 'delphi.nowcast_datasetname.sensors.sensor_update'
...
class UnitTests(unittest.TestCase):
  ...
  def test_validate_args(self):
    ...
    with self.subTest(name='only epiweek'):
      names, first, last, valid, test, target = validate_args(get_args(epiweek=201901))
    ...
    with self.subTest(name='valid names'):
      ...
      names, first, last, valid, test, target = validate_args(args)
      self.assertEqual(names, 'abc-def,foo-bar,123-321')

  def test_new_instance(self):
    """Create a SensorUpdate instance with default parameters."""
    self.assertIsInstance(SensorUpdate.new_instance(True, True, 'target'), SensorUpdate)
  ...
  def test_get_sensor_implementations(self):
    ...
    self.assertIn('isch', impls)
    self.assertTrue(callable(impls['isch']))

  def test_update_single(self):
    """Update a single sensor reading."""

    database = MagicMock()
    valid, test_week, target, name, location, value = True, 201820, 'target', 'name', 'loc', 3.14
    impl = MagicMock(return_value=value)
    implementations = {name: impl}
    sensor_update = SensorUpdate(valid, None, implementations, None, target)

    sensor_update.update_single(database, test_week, name, location)

    self.assertEqual(impl.call_count, 1)
    args, kwargs = impl.call_args
    self.assertEqual(args, (location, test_week - 1, valid, target))

    self.assertEqual(database.insert.call_count, 1)
    args, kwargs = database.insert.call_args
    self.assertEqual(args, (target, name, location, test_week, value))
  ...
  def test_get_location_list(self):
    ...
    with self.subTest(name='all'):
      self.assertEqual(get_location_list('all'), DatasetnameDataSource.ALL_LOCATIONS)
  ...
  #   sensor_update = SensorUpdate(True, database, implementations, epidata, 'target')
  ...
  def test_update_with_unknown_epiweek(self):
    ...
    sensor_update = SensorUpdate(True, database, implementations, None, 'target')
    ...
    self.assertEqual(args, ('ar', 201039, True, 'target'))

    self.assertEqual(impl2.call_count, 1)
    args, kwargs = impl2.call_args
    self.assertEqual(args, ('or', 201039, True, 'target'))

    self.assertEqual(database.insert.call_count, 2)
    args = [a for a, k in database.insert.call_args_list]
    self.assertEqual(args[0], ('target', 's1', 'ar', 201040, 1))
    self.assertEqual(args[1], ('target', 's2', 'or', 201040, 2))
  ...
  def test_update_with_failing_sensors(self):
    ...
    def impl(location, epiweek, valid, target):
    ...
    sensor_update = SensorUpdate(True, database, implementations, None, 'tar')
    sensor_update.update(sensors, 201820, 201820)

    self.assertEqual(database.insert.call_count, 3)
    args = [a for a, k in database.insert.call_args_list]
    self.assertEqual(args[0], ('tar', 's', 'ak', 201820, 1))
    self.assertEqual(args[1], ('tar', 's', 'ar', 201820, 2))
    self.assertEqual(args[2], ('tar', 's', 'az', 201820, 3))
